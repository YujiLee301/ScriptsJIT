...
def arg_parse():
    ...
    parser.set_defaults(model_type='Gated',
                        ...
                        pulevel=80,
                        lamb =7.4e-5,
                        training_path=r"../Pileup_GNN/data_pickle/dataset_graph_puppi_WjetsDR820000",
                        validation_path=r"../Pileup_GNN/data_pickle/dataset_graph_puppi_val_WjetsDR84000",
                        save_dir=r"testWjets/",
                        jet_type = "W",
                        num_select_LV = 7,
                        num_select_PU = 35,
                        act_fn = 'gelu',
...
def train(dataset, dataset_validation, args):
    ...
    while converge == False:
        ...
        """for data in training_loader:
            feature_with_mask = data.x
            print("feature_with_mask",feature_with_mask.size())
            for iter in range(rotate_mask):
                num_feature = data.num_feature_actual[0].item()
                print("num_feature ", num_feature)
                data.x = torch.cat((feature_with_mask[:, 0:num_feature],
                                     feature_with_mask[:,
        ...
        for batch in training_loader:
            ...
            for iter in range(rotate_mask):
                ...
                pred, d_da = model.forward(batch.x, batch.edge_index)
...
def test(loader, model, indicator, epoch, args, modelcolls, pathname):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            # max(dim=1) returns values, indices tuple; only need indices
            pred, pred_hybrid = model.forward(data.x, data.edge_index)
